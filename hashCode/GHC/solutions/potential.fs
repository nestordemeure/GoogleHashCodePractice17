        ...
        while sliceLength>=1 do
            ...
            for j = 0 to ((Array2D.length1 pizza)-1) do
                for i = 0 to ((Array2D.length2 pizza)-1) do
                    let sliceA = { left = i ; top = j-sliceHeight ; right = i+sliceLength; bottom = j; score=z}
                    let sliceB = { left = i-sliceHeight ; top = j-sliceLength ; right = i; bottom = j; score=z}
                    let sliceC = { left = i-sliceLength ; top = j ; right = i; bottom = i+sliceHeight; score=z}
                    let sliceD = { left = i ; top = j ; right = i+sliceHeight; bottom = j+sliceLength; score=z}
